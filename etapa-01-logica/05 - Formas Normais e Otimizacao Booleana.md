...
## 3. Aplicação na Automação: Otimização do Permissivo da Prensa (Setor 400)

Considere a expressão não otimizada de liberação de descida da prensa $P_{prensa}$ da nossa Envasadora. O copo deve estar presente ($s_4$), a temperatura ideal ($t_1$), a emergência NÃO acionada ($e_1$) ou um bypass ativo:
$$P_{prensa} = (s_4 \land t_1 \land \neg e_1) \lor (s_4 \land t_1 \land e_1 \land \text{byp}) \lor (s_4 \land \neg s_4 \land t_1)$$

1. O termo $(s_4 \land \neg s_4 \land t_1)$ contém contradição: $s_4 \land \neg s_4 \equiv 0 \implies 0 \land t_1 \equiv 0$.
2. Fatorando $(s_4 \land t_1)$:
   $$P_{prensa} \equiv (s_4 \land t_1) \land (\neg e_1 \lor (e_1 \land \text{byp}))$$
3. Pela regra de absorção distributiva $(\neg A \lor (A \land B) \equiv \neg A \lor B)$:
   $$P_{prensa\_otimizada} \equiv s_4 \land t_1 \land (\neg e_1 \lor \text{byp})$$

Reduzimos a quantidade de operações lógicas para diminuir a latência de varredura (*scan cycle*) do SCADA.

---

## 4. Entregável da Aula 05

* **Código Otimizado em Python:** Gerador e simplificador de expressões lógicas em FND/FNC para matrizes de segurança industrial. (Ver arquivo `.ipynb`).
